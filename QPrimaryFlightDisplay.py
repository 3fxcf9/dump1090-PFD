...
class QPrimaryFlightDisplay(WidgetClass):
    def __init__(self, parent=None):
        ...
        self.error = None
        self.zoom = 1
        self.change_tracked_flight = None
    ...
    def update_style(self, palette=None):
        ...
        self.bsbr = QColor(0x1E, 0x1F, 0x29)
        ...
        self.sky = QColor(0x1E, 0x1F, 0x29)
        self.ground = QColor(0x28, 0x2A, 0x36)
        self.font12 = QFont("Monaspace", 12 * z)
        self.font16 = QFont("Monaspace", 16 * z)
        self.font20 = QFont("Monaspace", 20 * z)
        self.errorFont = QFont("Monaspace", 35 * z)
    ...
    def mousePressEvent(self, event):
        """Detect click on the bottom left rectangle defined at draw_status"""
        h = self.geometry().height()
        s = self.scale
        rect = QRectF(0, h - 50 * s, 150 * s, 50 * s)
        if rect.contains(event.pos()):
            print("CLICK")
            if self.change_tracked_flight:
                self.change_tracked_flight()
    ...
    def draw_status(self):
        ...
        if self.flight is not None:
            ...
            painter.drawText(rect, Qt.AlignCenter, self.flight)

        if self.seen is not None:
            painter.setBrush(self.bsbr)
            painter.setPen(self.fg2)
            rect = QRectF(w - 150 * s, h - 50 * s, 150 * s, 50 * s)
            painter.drawRect(rect)
            painter.setFont(self.font16)
            painter.setPen(self.fg)
            painter.drawText(rect, Qt.AlignCenter, str(self.seen) + "s")

        if self.error is not None:
            rect = QRectF(0, h / 9, w, h / 9)
            painter.setFont(self.errorFont)
            painter.setPen(self.err)
    ...
    def draw_vspeed(self):
        ...
        painter.setFont(self.font16)
        painter.drawText(
            QRectF(x1, (y1 + y2) / 2 - 25 * s, x2 - x1 - 8 * s, 50 * s),
    ...
    def draw_altimeter(self):
        ...
        inc = 5
        r = per / inc
        x2 = 15 * w / 16 - 40 * s - 25 * s
        x1 = 15 * w / 16 - 120 * s - 40 * s - 25 * s
        y1 = h / 15 + 20 * s
        y2 = h - y1
        painter.setFont(self.font16)
        self.bsbr.setAlpha(128)
        ...
        painter.drawText(
            QRectF(x1, (y1 + y2) / 2 - 25 * s, x2 - 10 * s - x1, 50 * s),
            Qt.AlignVCenter | Qt.AlignRight,
            str(round(v)),
        )
    ...
    def draw_airspeed(self):
        ...
        for i in range(l1, l2 + 1):
            ...
            if (i % 2) == 0:
                ...
                painter.drawText(
                    QRectF(x1, (y1 + y2) / 2 + pos - 20 * s, x2 - 33 * s - x1, 40 * s),
                    Qt.AlignVCenter | Qt.AlignRight,
                    str(i * inc),
                )
            else:
        ...
        painter.setFont(self.font20)
        painter.drawText(
            QRectF(x1, (y1 + y2) / 2 - 25 * s, x2 - 33 * s - x1, 50 * s),
            Qt.AlignVCenter | Qt.AlignRight,
            str(round(v)),
        )
    ...
    def draw_skipskid(self):
        ...
        for a in ps:
            if (a < -pi / 4 or a > pi / 4) and not (
                b < -7 * pi / 32 or b > 7 * pi / 32
            ):
                continue
            trans = QTransform()
            trans.translate(w / 2, h / 2)
